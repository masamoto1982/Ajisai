...
export class Dictionary {
    ...
    renderBuiltinWords() {
        const container = this.elements.builtinWordsDisplay;
        container.innerHTML = ''; // コンテナをクリア

        // カテゴリごとにループ処理
        this.builtinWords.forEach(category => {
            // <h4>見出しを作成
            ...
            // そのカテゴリのワードボタンを描画
            this.renderWordButtons(container, category.words, false, true);
        });
    }
    ...
    updateCustomWords(customWordsInfo) {
        ...
        this.renderWordButtons(this.elements.customWordsDisplay, words, true, false);
    }

    renderWordButtons(container, words, isCustom, append = false) {
        // appendがfalseの場合、コンテナをクリア
        if (!append) {
            container.innerHTML = '';
        }
