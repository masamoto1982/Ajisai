...
import { DEMO_USER_WORDS, DEMO_WORDS_VERSION } from '../gui/demo-words';
import { Result, ok, err } from '../gui/functional-result-helpers';
...
export const createPersistenceService = (options: PersistenceServiceOptions): PersistenceService => {
    const { runtime, root = document, files, storage, showError, updateDisplays, showInfo } = options;
        ...
        try {
            ...
            if (state?.userWords && state.userWords.length > 0) {
                ...
                if (savedVersion < DEMO_WORDS_VERSION) {
                    ...
                    const newSampleWordNames = new Set(DEMO_USER_WORDS.map(w => w.name.toUpperCase()));
...
export const persistenceServiceUtils = {
    toUserWord,
    collectCurrentState,
    createExportData,
    buildExportFilename,
