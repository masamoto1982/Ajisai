...
> **Status: DRAFT — not yet canonical.** This subsection is a proposal under review. The design direction is fixed (the comparison path uses NICF, including `COMPARE-WITHIN`; see Section 7.4.1.1), but promotion is gated on the implementation prerequisite of Section 7.4.1.1 (NICF emission for every comparable representation, including `Gosper`). Until promoted, the regular continued fraction of Sections 4.2.1–4.2.4 is the sole basis for value identity, canonical form, display, and comparison.
...
> **Status: DRAFT — not yet canonical.** Proposal under review; depends on Section 4.2.5. The design direction is decided: the comparison path uses NICF throughout, including `COMPARE-WITHIN`, with the budget measured in semiregular terms (this resolves the former "budget unit" open question in favor of a single, uniform unit). Promotion is gated only on the implementation prerequisite below.
...
**Budget unit and `agreedPrefix` (decided).** One budget term is one **semiregular (NICF) term**, uniformly for the six relations (Section 7.4) and for `COMPARE-WITHIN` (Section 7.4.2). The `agreedPrefix` of a U result (Sections 4.5.0, 7.4.1) therefore counts matching NICF terms. Its contract is otherwise unchanged: a non-negative integer, monotone (a larger budget never decreases it), `0` for operands that differ at the first term, diagnostic-only (it never affects `truthValue`), and for `COMPARE-WITHIN` equal to the consumed `budget` on U. Because the unit is now uniformly NICF, there is no RCF-versus-NICF unit ambiguity to reconcile: a caller of `COMPARE-WITHIN` who names `budget = n` buys `n` semiregular terms, and that is the documented meaning. The correspondence between an NICF term count and an RCF term count is **not** observable and must not be relied upon.

**Promotion prerequisite (implementation).** Because the budget unit is now user-observable through `COMPARE-WITHIN`, NICF is not an optional per-implementation choice: a conforming implementation that adopts this section must emit NICF terms for **every** comparable scalar representation (Section 4.2.2) — `Rational`, `AlgebraicSqrt`, and crucially `Gosper`, which backs the results of arithmetic. Whether `Gosper` nodes can emit nearest-integer terms without prohibitive Möbius bookkeeping, and with the convergence advantage surviving that bookkeeping, is the open implementation question that gates promotion; it has not yet been demonstrated. Until it is, this section remains DRAFT and the regular CF of Sections 4.2.1–4.2.4 governs comparison.

On promotion, Section 15.3's comparison mandate is extended to require: (i) a reference set of decided comparisons produces an identical `truthValue` whether reasoned about via RCF or computed via NICF; (ii) `agreedPrefix` is monotone in the budget under NICF; and (iii) the normative tie-break of Section 4.2.5 yields the specified NICF digit sequence on the singular `1/2`-remainder cases.
