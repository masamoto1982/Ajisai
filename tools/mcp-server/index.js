...
export const TOOLS = [
  {
    ...
    description:
      ...
      "Its 78 Words cover arithmetic (ADD SUB MUL DIV MOD FLOOR CEIL ROUND ABS NEG MIN MAX SQRT), comparison (EQ NEQ LT LTE GT GTE), boolean logic (AND OR NOT), " +
      "vectors — arithmetic broadcasts element-wise — collections (SORT ORDER UNIQUE TALLY GROUP ZIP RANGE FILL TAKE DROP CONCAT REVERSE LENGTH GET PUT INDEX-OF MEMBER BSEARCH SHAPE RESHAPE FLATTEN DEPTH), " +
      "higher-order blocks (MAP FILTER FOLD SCAN ANY ALL RANK), text (CHARS JOIN TOKENIZE TRIM SEARCH REPLACE NUM STR), absence (NIL NIL? NIL-REASON ABSENT), and control (SELECT EXEC PROBE FAIL), plus DEF to name your own. " +
...
const RESOURCES = [
  ...
  // caller cannot do that from 78 separate probes.
...
export function createServer() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async ({ params }) => {
    ...
    if (params.name === "outcomes") return runAgent(args.source, "outcomes");
