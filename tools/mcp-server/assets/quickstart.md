...
| `COLLECT` | collection | Take N values off the stack and answer them as one Vector, first-pushed first: `1 2 3 3 COLLECT` is `[ 1 2 3 ]`. N must be a non-negative integer (`invalidCount`), and a stack holding fewer than N values is `stackUnderflow`. — e.g. `1 2 3 3 COLLECT` |
