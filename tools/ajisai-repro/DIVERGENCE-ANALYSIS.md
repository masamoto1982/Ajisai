...
（副次的論点として、headless CLI の JSON 出力経路が CHR-NIL を `''` と表示していたのは表示上の不整合。これは別途修正した。原因は CLI の `stack_display` が値固有の hint ではなく語の宣言出力ロール（CHR→TEXT）を positional hint として渡し、`format_with_hint` の Text 経路が NIL を空文字列として描画していたため。`rust/src/types/display.rs` の `format_with_hint` に「absence metadata を持つ operational NIL は hint に依らず `NIL` と描画する」ガード（既存の Unknown ガードと同型）を追加。正典 `Display`（値固有 hint を使用）は元から `NIL` を返しており不変。なお空文字列リテラル `''` も Ajisai では `NIL`（`EmptySequence`）として表現されるため、CLI でも正典どおり `NIL` と表示されるようになった。）
