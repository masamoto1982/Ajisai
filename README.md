...
Every numeric value is **exact**. Integer, fraction, decimal, and scientific-notation literals are convenient source forms for exact rationals, and `SQRT` produces exact algebraic irrationals carried in a multiquadratic normal form \(\sum_d c_d\sqrt d\). That is the present numeric domain, and it is the *witness* for a condition rather than a list chosen by taste: the domain is a field, closed under its arithmetic, whose equality and order an implementation decides in finite time through a normal form it exhibits. `SQRT` builds the field from a rational radicand rather than being an operation the field is closed under, and a real no exhibited normal form reaches is not an Ajisai value. Widening the domain means exhibiting a normal form that meets the condition, not amending the condition.
...
**Comparison is total.** Every comparison of two scalars answers `TRUE` or `FALSE` in finite time — not as a separate promise but as the deciding half of that condition. And a value is what it denotes, never how it was made: values built through different histories are the same value when they denote the same real, so `8 SQRT` equals `2 SQRT 2 SQRT +`.
...
| Path | Purpose |
| --- | --- |
| [`spec/`](spec/) | Canonical sources: the semantic kernel, the 65-Word registry, the shared laws, presentation, and the host protocol |
