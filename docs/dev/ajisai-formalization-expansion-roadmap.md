...
Phase 6(名前解決・辞書)・Phase 7(効果・観測)の完了で言語表層の中核へ拡大した。
残る `Absent` はレコード・文字列・並行性であり、Phase 8/9 として取り組む。各完了
フェーズの定義は形式化本体 §9-bis / §9-ter / §9-quater / §9-quinquies / §9-sexies、
法則は `rust/tests/{observation,desugar,contract_modifier,higher_order,structural,naming_resolution,effect_observation}_laws.rs`。
