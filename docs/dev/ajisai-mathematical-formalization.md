...
- 制御ディレクティブ `OR-NIL`(旧称VENT)は、後続フレーズを第二被演算子として
  束縛する二項適用として扱われる(§6.3)
...
`A OR-NIL B`(旧称VENT)は二項 `orNil(A, B)` として扱う:

```
orNil(⊥_r, b) = b          orNil(a, b) = a   (a が純粋値)
```

これはモナドの除去子(catch)であり、Bubble 層の唯一の handler。
誤差層(§1.3)には handler が無く、伝播して停止する(§11.4)。

> 注意:`OR-NIL` は**後続フレーズ B を第二被演算子として束縛する制御ディレクティブ**であって、
> 単純な後置スタック語ではない(実測:`5 OR-NIL 99 → 5`、`1 0 / OR-NIL 99 → 99`)。
...
- NIL モナド則: `⊥_r OR-NIL v ≡ v`、passthrough 自然性 `f(⊥_r) ≡ ⊥_r`
...
- **所見 I2(CONCAT は単一要素 top でアンダーフロー)— 解決:** `CONCAT` の **頂上
  オペランドが単一要素ベクタ**だと `[ 1 ] [ 2 ] CONCAT` は `StackUnderflow` になった
  (単一要素ベクタ頂上が特別扱い=spread される)。多要素頂上 `[ 1 ] [ 2 3 ] CONCAT`
  は正常だった。原因は「§7.1 の CONCAT arity」ではなく、正典が宣言しない**可変長
  呼び出し形**(`a b c 3 CONCAT`、負数で逆順)を実装が持ち、その個数をスタック頂上から
  sniff していたことにある。sniff が単一要素ベクタをその要素として読むため、
  `[ 1 ] [ 2 ]` は「上位 2 個を連結」と解釈された。可変長形を削除して正典どおりの
  `2 -> 1` に固定し、宣言済みだが到達不能だった `errorWhen: [nonVector]` を有効に
  した。文字列の `JOIN∘CONCAT` 則の ≥2 文字制約も解除済み(1 文字語を含む)。
