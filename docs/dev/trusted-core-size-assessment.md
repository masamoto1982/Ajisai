...
trusted core と見なし、他を派生と扱える境界」** である。

**訂正（2026-07、批判的レビュー A-4 を受けて）**: 上の段落の旧版は「懸念は
実務的に解消された」と主張していたが、これは二つの異なる面積を混同していた。
Minimal Core = 47 語が縮めるのは**言語同一性の面積**（何を保存すれば Ajisai が
Ajisai であるか）であって、**監査面積**ではない。メモリ安全・値の正しさ・
identity の健全性を確認するために読まなければならないコードは、依然として
実装全体 — 本改訂時点の実測で `rust/src` 76,277 行 / 264 ファイル — である。
47 語の境界はどの欠陥がその境界の外で起こるかを教えてくれない。したがって
正確な主張はこうなる: **言語同一性の核は 47 語に縮んだが、メモリ安全と
値正しさの監査面積は 76,277 行のままである**。両者を混同した「解消」表現は
撤回する。
