...
- Bubble Rule の説明では、「できなかった → `NIL` / そもそも使い方が違う → エラー」という言い方を採用する。「泡」という訳語は使わない(2026-09-14 に Reference から水のメタファーを撤去したため)。ただし LOOKUP 本文は UTF-8 English plain text を維持し、`Bubble/NIL` と書く。
