...
| `lexicon-emergence-experiment-work-order-2026-09.md` | Core を普遍文法として固定し、User 語彙を AI どうしに発見させる実験の作業指示書。伝達のボトルネック（辞書容量 K と世代交代）下で独立に再発明・収束する語と、使われない Core 語を測る。計測器 DIGEST の粒度を実測で固定（名前非依存・推移的・alias 正規化、`[ 2 ] *` と `2 *` はスカラー入力でだけ異なる）し、同一性を D0 構文／D1 観察／D2 契約の三段で数える。仮説 H1〜H5（H5 は方言化した発話が Core まで展開でき監査可能であること）、4 条件、同一モデルの事前分布を創発と誤認しない統制、Phase 0〜4。Phase 2 の経路 B ハーネス（Claude API、MCP 道具の中継、`submit`、費用上限）は実装済みで、API での再現は未実施（§6.1）。結果は §7.3 への候補提出に留め、正典を直接変えない | `[設計根拠]` |
| `lexicon-emergence-pilot-results-2026-09-23.md` | 語彙創発実験 Phase 1（パイロット、同一モデル 12 体・168 解答）の結果。全問正答で、正答率と圧縮率には情報がなかった。solo 条件だけで 34 類中 13 類が独立到達されたため、指示書 §0.5 の停止条件に該当し、H1 は判定不能。H5 に反例（`KEEP` をユーザー語に掛けた場合と、その本体を `EXEC` した場合で結果が違い、この形の語が継承されて 21 解答に伝播）。計測器の実測 4 件（DIGEST は束縛名を区別する → D0α、CONTRACT は `BIND` 本体で `inputs: variable`、探針不足による D1 の過結合、辞書の重複）と、Phase 2 の前提条件 | `[観察ノート]` |
| `word-arity-header-work-order-2026-09.md` | ユーザー語のアリティ宣言（パラメータ見出し `[ A B | body ]`）の作業指示書。語彙創発パイロットの H5 反例（`KEEP` をユーザー語に掛けると、本体への展開で意味が変わる）に対する所有者裁定の実装。見出しを持つ語は宣言した個数のオペランドを取り、空のスタックで本体を走らせ、`KEEP` は宣言したオペランドをちょうど残す。`BIND` による Core のみの展開が意味を保つ。`KEEP EXEC` がブロック内へ漏れる問題の修正、パラメータ名の位置符号化による内容同一性、契約推論での見出しと束縛名の扱い、`|` の予約。見出しなしの語は移行期間として現行のまま。Phase 1〜3 実施済み（Phase 2 はリポジトリ内の定義 312 箇所を `scripts/migrate-def-headers.mjs` で移行し、その過程で `check` と契約推論の欠陥 2 件を修正。Phase 3 は見出しを必須にし、動的 `KEEP` を削除し、`KEEP EXEC` をスタック全体を残す静的な規則にし、フレームより下を必ず読む本体を `DEF` 時に拒否する） | `[設計根拠]` |
