...
> 一致するかは未検証である。
>
> **検証追記（2026-09-06）**: `rust/src/kernel/mod.rs` のモジュール doc が「Phase 1
> skeleton、既存 runtime から未参照」と書いたままだったが、§10.11・§10.12 の記録どおり
> `interpreter::arithmetic` は scalar-scalar の `ADD`/`SUB`/`MUL`/`DIV` を
> `kernel::arithmetic` 経由で計算しており、実際には Phase 4 の配線が一部完了している。
> mod.rs 側のコメントは現状に合わせて修正済み（本書の記述はそのまま。上記の
> 「書き換えない」方針どおり）。あわせて mod.rs と `check-semantic-firewall.sh` が
> 引用していた「SPEC §20」は、削減前（約60コンセプト）の仕様番号を指しており現行の
> `spec/` には存在しない参照だったため、両ファイルとも `LANG.VALUES.DISJOINT` に
> 訂正した。§10 以降の Phase 進行状況が現行実装と一致するかは、この2点を除き依然
> 未検証である。
