...
The shared operand classifier lives in `rust/src/interpreter/scalar_fastpath.rs`.
Both arithmetic and comparison call it before their heavier StackTop pre-checks,
so the definition of "simple rational scalar" stays identical across `+ - * /`
and `< <= > >= = !=`.
