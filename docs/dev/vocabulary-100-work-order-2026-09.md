...
| `UPPER` / `LOWER` | A | Unicode のケース表を信頼カーネルに入れる覚悟が要る。ロケール問題も抱える。**2 枠空けば最優先で復帰**（テキスト処理の実用上は痛い欠落である）。→ §7.2 で復帰済み |
| `TAU`（2π） | — | `PI 2 MUL` と等コストの略記。`NEQ` を退役させた理由がそのまま当てはまる。詳細と実測は §7.3 |
...
| 6 | **実施済み** | `DEFINED?` `DIGEST` `CONTRACT` `FORMAT` `JSON-DECODE` `JSON-ENCODE` を追加し、`PROBE` を Record を返す語に再定義。語数 92（Kernel 54 / Standard 38）。反射の三語はいずれも Symbol を受け、String は新結末 `notASymbol`（`declared`、目撃者 1 件）——`LANG.DICTIONARY.ACYCLIC` の「どの語もテキストを Symbol に変えない」を守るため。`CONTRACT` と `PROBE` は `contract_record.rs` の一つの形を共有する: 共通鍵 `inputs` `outputs` `nil` `purity` `determinism` `cost` `effects` に加え、登録済み契約（Core 語）だけが `name` `tier` `consumption` `projection` `errors` `partiality` を、推論契約（User 語・ブロック）だけが `confidence` `gaps` を持つ。登録側の `projection` は投影**理由**（`NIL-REASON` が答える id）で、生成レジストリに `projection_reasons` を足した。`DIGEST` は Core 語に固定タグ付きの名前ダイジェスト、User 語に辞書の内容同一性、それ以外の値には観測ダイジェストと同じ符号化による denotation ダイジェスト（`observation_digest.rs` の `value_digest`）を答える。`FORMAT` は half-even 固定で、有理数は `Fraction::round_half_even`、Tier 1 は体の全順序で、Tier 2 は既定の比較予算で最終桁を決め、決まらなければ `undecidable` を投影する。`JSON-DECODE` は再帰しない（明示スタック）——ただし値表現の側が比較・表示・解放で再帰するため、入れ子は 512 段で打ち切り `spaceExhausted` を投影する（`json_decode.rs` の `MAX_NESTING`）。添付からの逸脱: (a) `DIGEST` は `passthrough` でなく `consumeNil`——NIL も denotation を持つ値であり、Vector の中の NIL がダイジェストに入る以上、単独の NIL も入るべき。Tier 2 を含む値では `undecidable` を投影するので `partiality` は `projecting`。(b) `FORMAT` は族 `output`（効果の族）でなく `text` 族・`cast` 分類——純粋な語であり、`STR`/`NUM` と同じ境界にいる。最終桁が決まらない場合があるので `projecting`。(c) `JSON-ENCODE` は `rejectNil` でなく `consumeNil`——NIL は `null` という像を持つ。(d) `JSON-DECODE` の投影理由は `invalidEncoding` に `spaceExhausted` を加えた二つ。(e) `CONTRACT` の族は添付どおり `control` に置いたが、`PROBE` と同じ理由（契約検査の語）による |
| 7 | **実施済み** | `POW` `GCD` `RATIO` `EXP` `LN` `SIN` `COS` `ATAN` を追加。語数 100（Kernel 55 / Standard 45）。所有者が Phase 7 開始時に §2.1 の分岐を再確認し、払い切りを採用。数値カーネルの拡張は `rust/src/types/exact/` の三つのモジュール: `series.rs`（方向付き固定小数点で有理点における厳密包囲——`exp` は引数の 2 分割と平方、`ln` は 2 進簡約と atanh 級数と ln 2 のキャッシュ、`atan` は π/2 − atan(1/x) と atan ½ + atan z の簡約、`sin`/`cos` は π の 512 ビット包囲による 2kπ 簡約とラグランジュ剰余）、`transcendental.rs`（各層の引数の包囲を単調関数は端点で、リプシッツ定数 1 の関数は中点 ± 半幅で写す）、`power.rs`（整数指数は底の層に留まり、p/2 は `SQRT` 経由で体に留まり、厳密な根は有理数、それ以外は `exp(y·ln x)`）。どの超越値も `Computable::from_base_bounds`——`PI` と同じく 512 ビットの基底包囲を一度だけ遅延計算し、各ステップは二進格子への外向き丸めだけ——なので、入れ子性と各ステップの費用上限は構成から従う。基底包囲の正しさは Python `decimal` で独立に計算した 60 桁の参照値と `transcendental_tests.rs` で照合した。添付からの逸脱: (a) `EXP` `SIN` `COS` は total でなく projecting——引数が機械の閾値（`exp` は 2^20、三角は 2^64）を超えると `spaceExhausted` を投影する。(b) `LN` `POW` `GCD` `RATIO` は Tier 2 の符号・整数性・有理性が予算内に決まらないとき `undecidable` を投影する（添付は domainMiss のみ）。(c) `POW` は添付の projection に `undecidable` と `spaceExhausted` を加えた四つ。付随修正: `ExactReal::div` の Tier 2 除数——従来は無条件に None（`divisionByZero`）だったものを、内部予算で 0 から分離できれば区間の逆数で答えるようにした（`10 LN 2 LN DIV` のため）。既知の限界: `PI SIN` や `PI COS` は 0 や −1 を包囲するが証明はしないので、`EQ` も `FORMAT` も `undecidable` を投影する——仕様どおりの振る舞いであり Reference に明記した |
...
残る——この分岐点は Phase 7 開始時にもう一度所有者に確認し、払い切りが再確認された）。

---

### 7.2 事後調整（全語の役割の見直しから）

全 Phase 完了後に 100 語の役割を一覧で見直し、所有者の指示で次の 3 点を実施した。
語数は 100 のまま（Kernel 54 / Standard 46）。

| 調整 | 内容 |
| --- | --- |
| `PROBE` を `CONTRACT` に統合 | 二語は「契約を Record で答える」同じ問いをオペランドの種類（ブロック / Symbol）で分けていただけであり、Symbol とベクトルは値として曖昧なく区別できる。`CONTRACT` がブロックも受け、走らせずに推論する。`PROBE` は退役語リストへ |
| `NEQ` を外す | `EQ NOT` と等コストの略記であり、記号別名 `!=` ごと退役（別名は 11 → 10）。空いた 1 枠と合わせて 2 枠 |
| `UPPER` `LOWER` を追加 | 待機リスト最上位。Unicode の既定の大小文字対応（ロケール非依存、`ß` → `SS` のような長さの変わる対応も適用）。対応表を `CHARS`/`JOIN` の定義で書くことはできないので Standard の operational |
| 丸め規則を一つに | `FORMAT` を half-even から「ちょうど半分は 0 から遠い側」に変え、`ROUND`・`QUANTIZE` と揃えた。Phase 6 の half-even は選択であって法則ではなく、同じ言語に丸め規則が二つあることの方が「丸めは一箇所」という主張を弱める |

### 7.3 alpha 期の採用規則と、その最初の適用

§7.2 の入れ替えは場当たりではない。以後の追加はこの規則で判断する。

> **規則。** alpha の間に語を足すときは、次のどちらかを満たすこと。
>
> 1. **言語内で書けない語であること。** 再帰がなく全域を要求するこの言語では、
>    `SORT` も `GCD` も `EXP` も定義としては書けない。書けない能力は語としてしか
>    持てないので、これは語数と引き換えにできない。
> 2. **さもなければ、同等以下の価値を持つ導出可能な語を一つ退役させること。**
>    語数は動かさない。

`UPPER` `LOWER` は条件 2 で入った（`NEQ` と `PROBE` を退役させた）。

**語数そのものを alpha の制約にしない理由。** 固定された語数は互換性の約束であり、
約束は 1.0 のものである。alpha とは語の集合をまだ動かせる期間のことなので、そこで
数を凍結すると、動かしてよいものを縛り、縛るべき採用規則の方が明文のないままになる。
`scripts/check-minimal-core.mjs` が 100 を検査するのは、追加を必ず意識させるための
引き金であって法則ではない。1.0 で凍結するのは、この規則と、その時点の語の集合の
両方である。そのとき初めて語数が約束になる。

**最初の適用: τ（2π）を検討し、入れないと決めた。** 条件 1 を満たさない
（`PI 2 MUL` で書ける）。条件 2 は持ち出すまでもない（退役させるべき語がない以上、
入れ替えではなく純増になる）。補強となる事実が三つある。

1. `NEQ` を退役させた理由、すなわち等コストの略記であることが、そのまま当てはまる。
2. e は既にワードではない。自然対数の底は `1 EXP` で得る。π と並ぶ頻度を持つ定数に
   名前を与えていない言語が、2π に名前を与える理由はない。
3. 数値的な利点がない。実測は次のとおり。

   | 式 | 結果 |
   | --- | --- |
   | `PI 2 MUL 10 FORMAT` | `'6.2831853072'` |
   | `PI 10 FORMAT` | `'3.1415926536'` |
   | `PI PI EQ` | `NIL` |
   | `PI 2 MUL 2 DIV PI EQ` | `NIL` |

   `PI 2 MUL` は π の 512 ビット基底包囲をちょうど 2 倍に写すだけで、精度も予算も
   失わない。下二行の `NIL` は導出の産物ではない。π を π 自身と比べてさえ予算内では
   決まらないのだから、τ を固有の語にしても比較が良くなることはない。これは Tier 2
   の比較の性質であって、綴りの問題ではない。

満枠であることがこの答えを出させた点を記録しておく。上限に余裕があれば、τ は
「何を退役させるか」を問われないまま入っていた。空き枠を残して alpha を進める案
（語数を 90 程度に抑える）は、この理由で採らない。加えてその案の費用は具体的で、
便益は仮定でしかない。能力を落とさずに削れるのは導出可能な 17 語だけなので、10 語
削るとはその句を毎回書くということであり、得られるのは現行規則が既に処理できている
判断を一度先送りすることだけである。

### 7.4 alpha の伸びしろ: 退役候補の列

§7.3 の規則の条件 2 は「導出可能な語を一つ退役させる」と言うだけで、どれを、どの
順で、とは言っていない。所有者の判断でその順を先に決めた。alpha 中に語を足す必要が
出たら、次の列の先頭と入れ替える。語数は 100 のまま動かさない。
...
| 2 | `GTE` | `LT NOT` | 不要 |
| 3 | `SUB` | `NEG ADD` | 不要 |
| 4 | `CEIL` | `NEG FLOOR NEG` | 不要 |
| 5 | `OR` | `NOT … NOT AND NOT`（ド・モルガン） | 片方で必要 |
| 6 | `QUANTIZE` | `MUL ROUND DIV` | 尺度で必要 |
| 7 | `ABS` | `X X MUL SQRT` | 必要 |
| 8 | `MIN` | `X Y X Y LT SELECT` | 必要 |
| 9 | `MAX` | `X Y Y X LT SELECT` | 必要 |
| 10 | `MOD` | `X Y DIV FLOOR Y MUL NEG X ADD` | 必要 |

順は失うものが少ない順である。「値の再利用」は、この言語に複製語がないことから来る
実際の費用を指す。`KEEP` は二項演算のオペランドを増やしてはくれない（`-7 KEEP MUL
SQRT` はスタック不足になる）ので、値を二度使う句は `BIND` で名前を付けることになる。
7 番以降を外す日は、利用者が計算した値に名前を付ける日でもある。

`ROUND` は導出可能だが列に入れていない。その Kernel の句だけがオペランドの符号で
分岐する（正なら `1/2 ADD FLOOR`、負なら `NEG 1/2 ADD FLOOR NEG`）ので、手で書いた
ときに最も間違えやすい。`MIN` と `MAX` は対で動かす。片方だけ残すのは、同じ問いに
語がある向きとない向きができるということである。

`TAKE` `DROP` `REVERSE` `INDEX-OF` `TRIM` `TOKENIZE` も導出可能だが列に入れていない。
一般の定義は書けるものの、`BIND` と `RANGE` と `GET` の集約を組む定型が要り、長い。
たとえば任意長の `REVERSE` は次のように書ける（実測）。

```
'V' BIND 0 V LENGTH 1 SUB 2 COLLECT RANGE 'I' BIND V V LENGTH 1 SUB I NEG ADD GET
→ [ 4/1 3/1 2/1 1/1 ]
```

operational の 29 語は列に入らない。言語内で書けない以上、退役は綴りではなく能力の
削除になるからである。`scripts/check-minimal-core.mjs` がこの不変条件を検査する。
列は 10 語、重複なし、全員が現役の導出可能 Standard であること。語を一つ入れ替えた
ら、列からその語を外すこと。

**この形を選んだ理由。** 所有者は当初、alpha を 90 語に抑えて 10 枠を空けておく案を
検討した。同じ伸びしろを、利用者が今日から費用を払う形で先取りするか、入れ替えが
実際に起きた日に払う形にするかの違いである。後者を採った。空き枠と待機列は、入れ替え
可能な候補が 10 語あるという点で機能的に等しく、待機列の方は候補が現れるまで誰も
`X Y X Y LT SELECT` と書かずに済む。
...
- ~~**`UPPER` / `LOWER` の欠落はテキスト処理で痛い。** 待機リスト最上位。~~
  解消済み（§7.2）。`NEQ` と `PROBE` の退役で空いた 2 枠に入れた。
