...
- 現状: `collect_builtin_definitions` と `collect_core_builtin_definitions` がタプル `(name, description, syntax)` を返すが、3番目は `BUILTIN_SYNTAX_PLACEHOLDER`（コミット `6a21818` で per-word の syntax 例が剥がされた）。
...
- **WASM タプル形状は `(name, description, syntax)` を維持する**。map / form / fold の分類は UI 境界には公開せず、Rust 内部では `WordShape` として型付きで保持する。
