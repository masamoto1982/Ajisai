...
`stackDisplay` は表示であって値ではない。本書の執筆時点では連分数表示を
**表示予算で打ち切った**文字列で、打ち切られた表示をハッシュすると異なる値が
同じダイジェストになった。その後、無理数の表示は正規形を一つのトークンで書く
形（`sqrt(2)`）に変わり打ち切りはなくなったが、表示と値を分ける原則は変わらない。
**必ず値そのものを符号化する。**
