...
後続 unit（join、sort-by、chunk）は別 PR。
...
- Core: 変更なし。新 Core 構文・新 primitive ゼロ。`DATA` は Module であり、`'DATA' IMPORT` で
  `CSV-PARSE` / `CSV-STRINGIFY` / `SELECT` / `WHERE` / `GROUP` が使える（未 import の `DATA@…` は
  他 Module と同様 Unknown）。
- 語は `Stability::Experimental`（新規 Module。API は後続 unit で発展しうる）。
- 生成物: `docs/word-manifest.json`（unit 3 で +1 = 222 語）、`SKILL.md`、
  `docs/formalization-coverage.json`（`module.data.group` を Sketched で追加、222/222 分類）、
...
- `DATA@JOIN` / `DATA@SORT-BY` / `DATA@CHUNK`。← unit 4–5
- per-field NIL reason（数値変換不可・join key 不在）の SPEC 協調導入。
