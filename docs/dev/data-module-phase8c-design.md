...
後続 unit（group、join、sort-by、chunk）は別 PR。
...
- Core: 変更なし。新 Core 構文・新 primitive ゼロ。`DATA` は Module であり、`'DATA' IMPORT` で
  `CSV-PARSE` / `CSV-STRINGIFY` / `SELECT` / `WHERE` が使える（未 import の `DATA@…` は他 Module と同様 Unknown）。
- 語は `Stability::Experimental`（新規 Module。API は後続 unit で発展しうる）。
- 生成物: `docs/word-manifest.json`（unit 2 で +2 = 221 語）、`SKILL.md`、
  `docs/formalization-coverage.json`（`module.data.select` / `where` を Sketched で追加、221/221 分類）、
