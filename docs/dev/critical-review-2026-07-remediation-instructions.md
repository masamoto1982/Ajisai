...
### R0-1 — word identity を SHA-256 へ移行する
...
**手順**
1. ハッシュ実装の選択。**推奨は `sha2` クレート**（RustCrypto,
   `default-features = false`, `wasm32-unknown-unknown` でビルド可）。
   Core の依存軽量方針（`rust/Cargo.toml:16-27` のコメント）と衝突するが、
   暗号プリミティブの自作を避ける原則が優先する。依存追加が方針上不可の場合の
   代替は FIPS 180-4 の SHA-256 を crate 内に実装し、NIST テストベクタで
   検証すること。**どちらを採るかは着手時に明示的に裁定し、その理由を
   本文書に追記する**。
2. `content_digest` を SHA-256 に差し替える。出力形式は `#` + 64 hex を維持
   （SHA-256 も 64 hex なので外形は不変）。
...
**非目標**
- BLAKE3 の採用（依存が重く、SHA-256 で要件を満たす）。
...
| BLAKE3 の採用 | **不採用** | SHA-256 で要件を満たし、依存が軽い |
