...
> Status: **Non-canonical** review note. 正典は `SPECIFICATION.md` のみ。
...
| 優先 | 項目 | 種別 | 本 PR |
| --- | --- | --- | --- |
| P0 | C: 無理数算術の近似退避を除去し CF ストリーム化 | 仕様準拠の実装修正 | 報告のみ |
| P0 | A: conformance を仕様各節へ拡張 | テスト拡充 | 一部着手 (19→37) |
| P1 | B: 真偽値を `truthValue` 軸として観測可能化 | 実装+仕様整合 | 報告のみ |
| P1 | D: `STAK` 修飾子の基本動作修正 | 実装修正 | 報告のみ |
