...
What was removed is the mapping — 器=Stack, 水=値, 流れ=dataflow, 波紋=`PRINT`, 泡=`NIL`, 決壊=error — used as an explanatory system across both documents. What remains, deliberately, is a single closing image in the "Why Ajisai" / 「なぜAjisaiを作ったのか」 section: the author's account of picturing water in a vessel while designing the language, and the hydrangea etymology behind the name. That image is authorial history, not a device a reader must decode to read the mechanics, and it is not leftover text to clean up. Do not reintroduce the mapping, and do not delete the closing image.)
