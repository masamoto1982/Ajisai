...
>
> - **上限の順序づけも完了。** 3 つの上限は独立したダイヤルではない。厳密計算では
>   大きな値を安く作れない（全桁・全項を実際に計算したということだから）ので、
>   各サイズ上限には到達最小作業量があり、プロファイルには
>   `work_to_reach(サイズ上限) < max_numeric_work` という整合条件が課される。
>   `max_algebraic_terms: 4096` はこれを破っており、超過する倍化が 16,799,744 単位
>   （予算 10,000,000）かかるため一度も発火したことがなかった。
>   **work 上限を上げるのではなくサイズ上限を下げて**満たした ——
>   work 上限は「実行」を、サイズ上限はその中の「1 個の値」を縛るので、階層を
>   反転させないため。`max_algebraic_terms` を **512** に（丸め値ではなく導出値：
>   `exactTerms` が responseBytes の 3.0%、4,096 項なら 26.5%）。`bigintBits` は
>   据え置き —— 78,913 桁は誰かが欲しがる値で、14% の余裕は記録すべき測定事実。
>   2 つを揃えて扱うのは正しさではなく見た目の整頓だった。
> - **これで宣言済み 10 上限すべてが実 source の boundary 被覆を持つ。**
> - **再発防止**: `rust/src/agent/profile_liveness_tests.rs`。どのサイズ上限も
>   work 予算の払える範囲になければビルドが落ちる。外部レビュー 3 往復が
>   見つけられなかったのは、誰も検査していなかったから。
>
> - **残っていた 1 件（コレクション系 Word の未課金）を解消した。**
>   設計と実測は `docs/dev/collection-word-billing-2026-08-13.md`、計測器は
>   `rust/examples/collection_word_calibration.rs`。課金対象は要素数ではない ——
>   同じ 16,000 要素の `UNIQUE` がデータ次第で 0.52 ms にも 682 ms にもなり
>   （1,300 倍）、入れ子要素は先頭で違うか末尾で違うかで 41 倍違い、代数的要素
>   1 個の比較は機械語の 500 倍（3.0 µs 対 5.8 ns）だった。価格は
>   「操作回数 × 実測した 1 要素の値段」で、複製・比較族は入口で前払い、
>   走査族だけは「これまでの相異なり数」ぶんを要素ごとに前払いする ——
>   走査の値段はオペランドの**中身**の関数で、事前に確定しないため。
>   `collectionWork`（20,000,000）を新設した。`numericWork` に相乗りしなかったのは
>   単位が違って正直な換算ができないことと、`numericWork` の名でコレクションの
>   二次を拒否すると P0-3 と同じ「指示が逆になる」問題が起きるため。
>   `[ 0 99999 ] RANGE UNIQUE` は 45.5 秒 → **164 ms で名前つき拒否**。
> - **副産物**: `LENGTH` は O(1) を宣言しながら `extract_vector_elements` が
>   要素ベクタ全体を深く複製してから `.len()` を呼んでいた（100,000 要素で 18 ms）。
>   実測で出てきたので直した。
> - **`wallTimeMs` の boundary 被覆はこれで失われ、`hostGate` へ移した。** 上の
>   over ケースがまさに `wallTimeMs` の over ケースだったからである。これは後退では
>   なく前進で、名前つき上限が全経路を覆った結果、上限が許す最も遅いプログラムが
>   約 1.4 秒になり、5,000 ms はその**和**に対する backstop になった。adapter が
>   持つ締切なので adapter 経由（1 ms の締切を入れた backend）で試験する。
>
> - **P1-2 に着手し、原案の診断を実測で置き換えた。** 原案は「§2-2 と §4 の前方参照の
>   欠落」と「8 KB 目標での分割」を挙げていた。前者は直した（§2-2 に「SQRT の結果は §4 を
>   読め」の一文）。**後者は実施しないと判断した** —— quickstart は resource で、読むと
>   決めた相手にしか配られないので、26 KB は読まれない限り無料である。そして baseline が
>   示した失敗は「最初の呼び出しの前に情報が足りない」であって「後から多すぎる」ではない。
>   分割は逆向きに働く。
> - **本当の P1-2 は別のところにあった。** `tool_choice: auto` では、最初の呼び出しの前に
>   読まれるのは 4 つの tool description だけである。`compute` の説明は数値領域しか
>   挙げておらず、コレクション・高階・text に一言も触れていなかった —— 呼ばなかった
>   21 件のうち 12 件がコレクションだったのはそのためで、モデルは説明どおりに判断していた。
>   残る 46 件は `vec-add` `group-by` `nil-or` `dict` といった**存在しない名前**での
>   registry 探索に費やされていた。使う気がなかったのではなく、名前を知らなかった。
>   説明に全 family と Word 名を入れ、`ajisai://vocabulary` を指すようにした。
>   `tool-description.test.js` が「宣言済み family は必ず 1 語以上名指しされる」
>   「説明が挙げる名前は必ず実在する」を固定する（後者は baseline が測ったのと同じ失敗を
>   こちら側がやらないための検査）。
> - **ただし効果は未測定である。** baseline を取った鍵はローテーション済みで、after
>   キャプチャを回していない。仕組みは揃っているので、`eval:capture` と `score-traces.js`
>   を同じ 130 プロンプトに対して回せば比較できる。それまでは「変更した」であって
>   「改善した」ではない。
>
> **これで既知の未課金経路はコレクション系を含めて全て塞がった。** 残るのは
> text 系（`STR` の O(桁²) 10 進変換）で、`bigintBits` が許す幅では数十 ms 規模、
> 今回塞いだ穴とは桁が違うので follow-up として記録に留める。
