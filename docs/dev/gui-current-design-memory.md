...
- Stack area has visual highlight modes triggered by code content, scanned as whole whitespace-delimited modifier tokens so the combined forms (`.,,`, `..,,`) and the `;`/`;;` sugar are recognized, and decimals like `.5`/`5.` never false-trigger. Both modifier axes ride on one **underline** channel on the operand nodes (an underline keeps the value's own color legible where a background tint washed it out) — the underlined set is the target, the underline's color and style are the consumption fate (a non-operand can never be consumed, so no extra marker is needed):
  - Target axis (which items are underlined): `..` (or `;;`) underlines every stack item (STAK), otherwise the default `.` (TOP) underlines only the top item — "which values are the operands".
  - Consumption axis (the underline color and style): the default `,` (EAT) is a dashed warm-red underline (operands are removed), `,,` (or `;;`) a solid teal-green underline (KEEP — operands are retained) — "what becomes of them". The pair is separated by both hue and dashed-vs-solid style (`--color-consume-eat` / `--color-consume-keep` in `tokens.css`) so it reads apart under color-vision deficiency. The two axes are independent, mirroring SPEC §6.3.
