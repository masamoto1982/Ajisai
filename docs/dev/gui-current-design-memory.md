...
- The Stack area renders values as KaTeX-typeset mathematics **by default**: rationals as fractions, rank-1/rank-2 numeric vectors and tensors as matrices, `approximate` nodes with a leading approx sign. The "Math view" toggle (top-right of the panel, persisted in `localStorage`) switches back to the canonical protocol strings.
