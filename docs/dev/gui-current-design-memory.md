...
- **The Dictionary is a catalogue on a phone, not a keyboard.** A word button types its name into the editor on desktop only — `onWordClick` is a no-op on mobile (`gui-application.ts`), where tapping a word raises its summary and example and nothing else. So the mobile chip is sized to show the vocabulary rather than to be pressed: the 100 Core Words fit in 15 rows at 360px and 14 at 390px, inside the list box, with no scrolling. The text is *not* smaller than desktop (0.75rem either way) — what shrank is the padding and margin the 44px floor was inflating. Two rules have to be turned off for that: the mobile block restates `font-size`, `padding` and `margin` to opt out of the `@container panel (max-width: 26rem)` rule, which grows chips for a slim desktop column and cannot tell one from a phone. Below 340px the type does step down to 0.65rem, the one place where seeing all of it outranks a larger glyph. A 320x568 screen (2016-era 4in) still scrolls: its list box is ~205px and 100 chips cannot be legible in that.
