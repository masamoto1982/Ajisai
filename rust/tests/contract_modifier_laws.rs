...
fn keep_preserves_the_operands_of_a_user_word_call_not_its_body_literals() {
    assert_eq!(
        obs("[ X | X 2 * ] 'TWICE' DEF 5 KEEP TWICE"),
        vec!["5/1", "10/1"]
    );
    assert_eq!(
        obs("[ X | X 1 - ] 'DEC' DEF 5 KEEP DEC"),
        vec!["5/1", "4/1"]
    );
    // A binary User Word retains both operands, matching the Core Word law.
    assert_eq!(
        obs("[ X Y | X Y + ] 'PLUS' DEF 3 5 KEEP PLUS"),
        vec!["3/1", "5/1", "8/1"]
    );
    ...
    assert_eq!(
        obs("[ X Y | X Y + 5 - ] 'F' DEF 3 5 KEEP F"),
        vec!["3/1", "5/1", "3/1"]
    );
...
fn keep_is_bifurcation_across_abstraction() {
    ...
    expected.extend(obs("[ X Y | X Y + ] 'PLUS' DEF 3 5 PLUS"));
    assert_eq!(expected, obs("[ X Y | X Y + ] 'PLUS' DEF 3 5 KEEP PLUS"));
}
...
fn keep_over_a_total_identity_word_copies_any_value() {
    let id = "[ X | X 1 COLLECT [ 0 ] GET ] 'ID' DEF ";
    assert_eq!(
