...
# スタック全体の並べ替え
a b c [ -1 -2 -3 ] .. REORDER     # → c b a（逆順）
a b [ 1 0 ] .. REORDER            # → b a（2要素交換）
a b c [ 1 2 0 ] .. REORDER        # → b c a（左ローテート）
a b c [ 2 0 1 ] .. REORDER        # → c a b（右ローテート）
...
## 応用例
# Layer 1 標準ライブラリでの定義例
# GET末尾:    [ [ -1 ] .. GET ]（末尾要素をコピー）
# REMOVE末尾: [ [ -1 ] .. REMOVE ]（末尾要素を削除）
# SWAP2:      [ [ -2 -1 ] .. REORDER ]（最後の2要素を交換）
# LROT3:      [ [ -3 -1 -2 ] .. REORDER ]（a b c → b c a）
