...
## 動作
- 上から順に条件を評価
- 最初にtrueになった条件の次行（アクション）を実行
- すべての条件がfalseの場合、>>> のデフォルトアクションを実行
...
- 最後は必ず >>> で終わる必要があります
- >> の数は偶数、>>> は1つだけ"#.to_string(),

        ">>>" => r#"# >>> - シェブロン分岐（デフォルト）

## 機能
シェブロン分岐のデフォルトケースを定義します。
すべての >> 条件がfalseの場合に実行されます。
...
## 注意
- シェブロン分岐の最後に必ず1つだけ必要です
- >>> が複数ある場合はエラーになります"#.to_string(),
...
- 20Hz未満、20kHz超の周波数には警告が出力されます"#.to_string(),
        ...
        "SLOT" => r#"# SLOT - スロットデュレーション設定

## 機能
1スロットあたりの秒数を設定します。
PLAYワードで再生される音の基準時間単位を変更します。
...
## 使用例
# 直接指定
0.5 SLOT               # 1スロット = 0.5秒（デフォルト）
0.25 SLOT              # 1スロット = 0.25秒（速い）
1 SLOT                 # 1スロット = 1秒（遅い）

# 分数で指定（精度保持）
1/4 SLOT               # 1スロット = 0.25秒
1/8 SLOT               # 1スロット = 0.125秒
3/4 SLOT               # 1スロット = 0.75秒

# BPMワードの定義例
: [ 60 ] SWAP / SLOT ; 'BPM' DEF
120 BPM                # 120 BPM → 0.5秒/スロット
60 BPM                 # 60 BPM → 1秒/スロット

# 音長との組み合わせ
0.125 SLOT             # 32分音符を基準
[ 440/4 550/2 660/1 ] PLAY   # 4分音符, 8分音符, 32分音符

## 注意
- グローバル設定です（全ての再生に影響）
- 正の値のみ有効（0以下はエラー）
- 極端に小さい（<0.01秒）/大きい（>10秒）値は警告"#.to_string(),
...
## 注意
- グローバル設定です（全ての再生に影響）
- GAIN-RESET でデフォルトに戻せます"#.to_string(),

        "GAIN-RESET" => r#"# GAIN-RESET - 音量リセット
...
[ 440 ] PLAY           # 30%で再生
GAIN-RESET
[ 440 ] PLAY           # 100%で再生"#.to_string(),

        "PAN" => r#"# PAN - 定位（パンニング）設定
...
- グローバル設定です（全ての再生に影響）
- ヘッドフォンで効果が明確になります
- PAN-RESET でデフォルトに戻せます"#.to_string(),

        "PAN-RESET" => r#"# PAN-RESET - 定位リセット
...
[ 440 ] PLAY           # 左から再生
PAN-RESET
[ 440 ] PLAY           # 中央から再生"#.to_string(),

        "FX-RESET" => r#"# FX-RESET - 全エフェクトリセット
...
## 使用法
FX-RESET
→ GAIN=1.0, PAN=0.0 に設定

## 使用例
0.3 GAIN -0.7 PAN
[ 440 ] PLAY           # 30%音量、左寄りで再生
FX-RESET
[ 440 ] PLAY           # 100%音量、中央で再生

## 現在リセットされるエフェクト
- GAIN → 1.0（100%音量）
- PAN → 0.0（中央）"#.to_string(),
...
- 暗号的用途（パスワードハッシュなど）には専用アルゴリズムを推奨"#.to_string(),
        ...
        "EXEC" => r#"# EXEC - ベクタをコードとして実行

## 機能
ベクタ（またはスタック全体）をAjisaiコードとして解釈し、実行します。
メタプログラミングを可能にする強力なワードです。
...
## 使用例（StackTopモード）
[ 1 1 + ] EXEC           # → [ 2 ]（1 1 + が実行される）
[ [ 2 ] [ 3 ] * ] EXEC   # → [ 6 ]（[ 2 ] [ 3 ] * が実行される）
[ DUP * ] 'SQ' DEF       # カスタムワードを定義
[ 4 ] [ [ 3 ] SQ ] EXEC  # → [ 4 ] [ 9 ]

## 使用例（Stackモード）
[ 1 ] [ 1 ] '+' .. EXEC  # → [ 2 ]
# スタック上の [ 1 ], [ 1 ], '+' がコードとして実行される

[ 2 ] [ 3 ] '*' .. EXEC  # → [ 6 ]
# スタック上の [ 2 ], [ 3 ], '*' がコードとして実行される

## 動作詳細
1. ベクタ内の要素をソースコード文字列に変換
2. トークナイズ（カスタムワードを含む）
3. 変換されたトークン列を実行

## 注意
- Stackモードでは、実行前にスタックの内容は全て消費されます
- 非同期操作（WAITなど）は EXEC 内では使用できません
- 無効なコードはトークナイズまたは実行時にエラーになります"#.to_string(),

        "EVAL" => r#"# EVAL - 文字列をパースして実行

## 機能
文字列（またはスタック上の文字コード列）をAjisaiソースコードとして
パースし、実行します。
...
## 使用法（Stackモード）
文字コード1 文字コード2 ... .. EVAL
→ スタック上の数値を文字コードとして結合し、パースして実行

## 使用例（StackTopモード）
'1 1 +' EVAL              # → [ 2 ]
'[ 2 ] [ 3 ] *' EVAL      # → [ 6 ]
'[ 10 ] [ 20 ] + PRINT' EVAL  # "[ 30 ]" を出力

## 使用例（Stackモード）
# ASCII: 49='1', 32=' ', 50='2', 43='+'
[ 49 ] [ 32 ] [ 50 ] [ 32 ] [ 43 ] .. EVAL   # → [ 3 ]
# 文字コード列から "1 2 +" が復元され実行される

# 動的にコードを構築
[ 91 ] [ 53 ] [ 93 ] .. EVAL  # → [ 5 ] ("[5]" を実行)

## 動作詳細
1. StackTopモード: スタックトップの文字列を取得
2. Stackモード: スタック全体を文字コード列として結合
3. 結果の文字列をトークナイズ
4. トークン列を実行

## EXECとの違い
- EXEC: 既存のValue（ベクタ）をコードとして解釈
- EVAL: 文字列をパースしてからコードとして解釈
...
## 注意
- Stackモードでは、実行前にスタックの内容は全て消費されます
- 非同期操作（WAITなど）は EVAL 内では使用できません
- 構文エラーがあるとトークナイズ時にエラーになります"#.to_string(),

        // ============================================================================
        // 形状操作
        // ============================================================================

        "SHAPE" => r#"# SHAPE - ベクタの形状を取得

## 機能
ベクタの各次元のサイズを取得し、1次元ベクタとして返します。
...
## 消費モード
- `. ,`（デフォルト）: 入力を消費し、形状をプッシュ
- `. ,,`: 入力を保持し、形状を追加

## NIL安全
NIL入力の場合、NILを返します（Map型のNIL伝播）。

## エラー
- `..`（Stackモード）はサポートされません"#.to_string(),

        "RANK" => r#"# RANK - ベクタの次元数を取得

## 機能
ベクタのネスト深さ（次元数）をスカラーとして返します。
...
- `..`（Stackモード）はサポートされません"#.to_string(),
        ...
        "RESHAPE" => r#"# RESHAPE - ベクタの形状を変更

## 機能
ベクタの要素を指定した形状に再配置します。
元のデータ数と新しい形状の要素数は一致している必要があります。

## 署名型
Form型（入力の構造を変換する）

## 使用法
data_vector shape_vector RESHAPE → reshaped_vector
...
## 消費モード
- `. ,`（デフォルト）: 両オペランドを消費し、結果をプッシュ
- `. ,,`: 両オペランドを保持し、結果を追加

## 制限
- 最大3次元まで（Rule of Three）
- データ数と形状の積が一致しない場合はエラー

## エラー
- `..`（Stackモード）はサポートされません
- 形状の要素は正の整数でなければなりません"#.to_string(),

        "TRANSPOSE" => r#"# TRANSPOSE - 2次元ベクタの転置
...
## 使用法
2d_vector TRANSPOSE → transposed_vector

## 使用例
[ [ 1 2 3 ] [ 4 5 6 ] ] TRANSPOSE  # → { ( 1 4 ) ( 2 5 ) ( 3 6 ) }

## 消費モード
- `. ,`（デフォルト）: 入力を消費し、転置結果をプッシュ
- `. ,,`: 入力を保持し、転置結果を追加

## NIL安全
NIL入力の場合、NILを返します（Form型: NIL = 空集合）。

## エラー
- 2次元ベクタのみ対応（1次元や3次元はエラー）
- `..`（Stackモード）はサポートされません"#.to_string(),

        "FILL" => r#"# FILL - 指定値で埋めたベクタを生成

## 機能
指定した形状のベクタを生成し、すべての要素を指定した値で埋めます。
引数ベクタの最後の要素が埋める値、それより前の要素が形状を表します。

## 署名型
Form型（新しい構造を生成する）

## 使用法
[ shape... value ] FILL → filled_vector
...
## エラー
- `..`（Stackモード）はサポートされません
- 形状の要素は正の整数でなければなりません
- 埋める値はスカラーでなければなりません"#.to_string(),
