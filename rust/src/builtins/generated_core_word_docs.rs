...
pub(crate) const GENERATED_CORE_WORD_DOCS: &[GeneratedCoreWordDoc] = &[
    ...
    GeneratedCoreWordDoc {
        name: "SQRT",
        family: "exactArithmetic",
        summary: "Exact square root of a non-negative rational, element-wise over a vector. The result is carried in multiquadratic normal form and compares with no rounding. A negative radicand projects to NIL.",
    ...
    GeneratedCoreWordDoc {
        ...
        hover_summary: "PUT — replace the element at an index",
    ...
    GeneratedCoreWordDoc {
        ...
        hover_summary: "AT — the value under a key, or the absence notFound",
    ...
    GeneratedCoreWordDoc {
        name: "WITHOUT",
        family: "record",
        summary: "A copy of a Record with one key removed: `R 'x' WITHOUT`. Removing a key the Record does not hold is not an identity but the absence `notFound` — the same discipline `GET`, `TAKE` and `PUT` keep for a position outside the Vector, so a misspelled key cannot pass silently. The other keys keep their order. A non-Record first operand is an ERROR.",
