...
pub(crate) const GENERATED_CORE_WORD_DOCS: &[GeneratedCoreWordDoc] = &[
    ...
    GeneratedCoreWordDoc {
        ...
        role: "Comparison-selected operand; an undecidable Tier 2 pair yields UNKNOWN.",
    ...
    GeneratedCoreWordDoc {
        ...
        role: "Ordering over exact scalars; an undecidable pair yields UNKNOWN.",
    ...
    GeneratedCoreWordDoc {
        name: "RANK",
        category: "higher-order",
        summary: "MAP at a stated depth. RANK descends that many levels into the vector, stopping early at a leaf, and evaluates the block once on each value it reaches, in index order, rebuilding the structure above them: `[ [ 1 2 ] [ 3 4 ] ] 2 [ 10 MUL ] RANK` is `[ [ 10 20 ] [ 30 40 ] ]`, depth 1 is exactly MAP, and depth 0 evaluates the block once on the whole vector. The block runs on an isolated frame holding the value reached and must leave one result (LANG.SOURCE.FRAME). A depth that is not a non-negative integer is ERROR(invalidCount). This is how a block reaches an inner axis without a second modifier axis.",
        role: "Higher-order primitive: evaluate a block on every value at a stated depth of a vector.",
        stack_effect: "[ vec ] [ n ] [ body ] -> [ result ]",
        hover_summary: "RANK — apply a block at a stated depth",
        hover_syntax: "[ [ 1 2 ] [ 3 4 ] ] 2 [ 10 MUL ] RANK",
    },
    GeneratedCoreWordDoc {
        name: "CHARS",
        category: "cast",
