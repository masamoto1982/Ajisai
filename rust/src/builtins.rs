...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("WAIT", "Execute custom word after delay (ms). Usage: 'WORD' [ ms ] WAIT", "Control"),
...
- コロンの数は奇数個になります（条件:アクション:条件:アクション:デフォルト）"#.to_string(),
        ...
        "TIMES" => r#"# TIMES - カスタムワードをN回繰り返し実行

## 機能
指定されたカスタムワードを指定回数繰り返し実行します。
ビルトインワードには使用できず、カスタムワード専用です。
...
## エラーケース
'PRINT' [ 3 ] TIMES    # エラー：ビルトインワードには使用不可
'UNKNOWN' [ 5 ] TIMES  # エラー：存在しないワード

## 注意
- ワード名はシングルクォート文字列で指定
- 実行回数は単一要素ベクタの整数で指定
- カスタムワードのみに使用可能（ビルトインワードには使用不可）
- 各実行の結果はスタックに積まれます"#.to_string(),

        "WAIT" => r#"# WAIT - カスタムワードを遅延実行（予定機能）
...
## 現在の実装状態
⚠️ 現在は実際の遅延は実装されておらず、デバッグメッセージを出力して即座に実行されます。
将来的に非同期処理による遅延実行が実装される予定です。
...
## 使用例
# 1000ms後にカスタムワードを実行（予定）
[ '[ 100 ]' ] 'HUNDRED' DEF
'HUNDRED' [ 1000 ] WAIT
# → 現在は即座に [ 100 ] が実行される（デバッグメッセージ付き）

## エラーケース
'PRINT' [ 100 ] WAIT    # エラー：ビルトインワードには使用不可
'UNKNOWN' [ 500 ] WAIT  # エラー：存在しないワード

## 注意
- ワード名はシングルクォート文字列で指定
- 遅延時間は単一要素ベクタの整数（ミリ秒）で指定
- カスタムワードのみに使用可能（ビルトインワードには使用不可）
- 現在は遅延なしで即座に実行されます"#.to_string(),
