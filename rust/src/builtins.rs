...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("1D", "1次元ブラケットを入力｜1D → { }、[ 2 ] 1D → { } { }", "Input Helper"),
        ("2D", "2次元ブラケットを入力｜2D → { ( ) }、[ 2 ] 2D → { ( ) ( ) }", "Input Helper"),
        ("3D", "3次元ブラケットを入力｜3D → { ( [ ] ) }、[ 2 ] 3D → { ( [ ] [ ] ) }", "Input Helper"),
...
## 機能
1次元（可視の最外殻）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定すると、複数の1次元構造をスタックに配置します。
...
## 機能
2次元用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定すると、内部要素の数を増やせます。
...
## 機能
3次元（可視の限界）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定すると、最内殻要素の数を増やせます。
...
## 注意
- これは入力支援機能です
- Ajisaiは0次元（スタック）を含めて4次元、可視は3次元までをサポートします
- 繰り返し数は正の整数である必要があります"#.to_string(),
