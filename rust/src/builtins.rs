...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("FRACTIONSORT", "分数専用高速ソート（Introsort）｜[ 1/2 1/3 2/3 ] FRACTIONSORT → [ 1/3 1/2 2/3 ]", "Sorting"),
...
- 整数のみサポート"#.to_string(),
        ...
        "FRACTIONSORT" => r#"# FRACTIONSORT - 分数専用高速ソート

## 機能
ベクタまたはスタック内の数値を昇順にソートします。
Introsortアルゴリズムを使用し、分数の比較には除算を避けて
クロス乗算（a/b < c/d ⟺ a×d < b×c）を使用することで、
高速かつ正確なソートを実現します。
...
### 1. 分数の比較方法
通常、分数を比べるには割り算が必要です：
  1/2 と 1/3 を比較 → 1÷2=0.5、1÷3=0.333... → 0.5 > 0.333...

FRACTIONSORTは割り算を使わず、掛け算だけで比較します：
  1/2 と 1/3 を比較 → 1×3=3、2×1=2 → 3 > 2 なので 1/2 > 1/3

これは「a/b < c/d」と「a×d < b×c」が同じ意味だという数学の性質を利用しています。

### 2. Introsortアルゴリズム
3つの技を組み合わせた賢いアルゴリズムです：
- QuickSort: 普段はこれで高速処理
- HeapSort: データの並びが悪い時の保険
- InsertionSort: 少量データ用の最適化

データの状態に応じて自動的に切り替わるので、どんな場合でも速く動きます。

### 3. 整数も速い
Ajisaiでは整数も内部的に分数として扱われます（分母が1の分数）：
  2 → 2/1、8 → 8/1、18 → 18/1

分母が1なので実質的に分子だけの比較になり、とても効率的です。
...
## 使用例
# 整数のソート
[ 32 8 2 18 ] FRACTIONSORT         # → [ 2 8 18 32 ]

# 分数のソート
[ 1/2 1/3 2/3 ] FRACTIONSORT       # → [ 1/3 1/2 2/3 ]

# 混在もOK
[ 3 1/2 2 1/4 ] FRACTIONSORT       # → [ 1/4 1/2 2 3 ]

# Stackモード
64 25 12 22 11 .. FRACTIONSORT     # スタック全体が昇順に

## なぜ速いのか？
- 掛け算は速い: 現代のコンピュータは整数の掛け算が得意
- 割り算を避ける: 割り算は掛け算より10倍以上遅い
- 変換しない: 浮動小数点数に変換する無駄な処理がない
- 実績あるアルゴリズム: 何十年もかけて最適化されたIntrosort

## エラーケース
[ 'a' 'b' ] FRACTIONSORT           # エラー：数値以外を含む
[ 1 ] FRACTIONSORT                 # エラー：1要素のみ（変化なし）
[ 1 2 3 ] FRACTIONSORT             # エラー：既にソート済み（変化なし）
