...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("LEVEL", "テンソル/ベクタを1次元に平坦化｜[ [ 1 2 ] [ 3 4 ] ] LEVEL → [ 1 2 3 4 ]", "Tensor"),
...
- 変化が生じない場合は必ずエラーになります"#.to_string(),
        ...
        "LEVEL" => r#"# LEVEL - テンソル/ベクタを1次元に平坦化

## 機能
テンソルまたはネストされたベクタを1次元のテンソルに平坦化します。
多次元テンソルの全要素を行優先順序（row-major order）で並べた1次元テンソルを返します。
...
## 注意
- 行優先順序で平坦化されます
- 1次元テンソルの場合は変化なしでエラーにはなりません"#.to_string(),
...
- これは実行可能なワードではなく、入力支援機能です"#.to_string(),
        ...
        "SCALAR" => r#"# SCALAR - 1次元ブラケット入力

## 機能
1次元（スカラー）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定できます。
...
- 繰り返し数は正の整数である必要があります"#.to_string(),
        ...
        "VECTOR" => r#"# VECTOR - 2次元ブラケット入力

## 機能
2次元（ベクトル）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定できます。
...
- 繰り返し数は正の整数である必要があります"#.to_string(),
        ...
        "MATRIX" => r#"# MATRIX - 3次元ブラケット入力

## 機能
3次元（行列）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定できます。
...
- 繰り返し数は正の整数である必要があります"#.to_string(),
        ...
        "TENSOR" => r#"# TENSOR - 4次元ブラケット入力

## 機能
4次元（テンソル）用のブラケット構造をInputエリアに挿入します。
オプションで繰り返し数を指定できます。
...
- !はDEL/DEFの前に置く必要があります（後置記法のため）"#.to_string(),
        ...
        "SCAN" => r#"# SCAN - 中間結果を保持する畳み込み

## 機能
FOLDと同様に初期値から畳み込みを行いますが、
各ステップの中間結果をすべて保持したベクタを返します。
Haskellの scanl に相当する高階関数です。
...
[ 1 2 3 4 ] [ 1 ] '*' SCAN     # → [ 1 2 6 24 ]
# 1*1=1, 1*2=2, 2*3=6, 6*4=24

## FOLDとの比較
[ 1 2 3 4 ] [ 0 ] '+' FOLD     # → [ 10 ]（最終結果のみ）
[ 1 2 3 4 ] [ 0 ] '+' SCAN     # → [ 1 3 6 10 ]（全中間結果）

## 注意
- 結果のベクタ長は入力ベクタと同じ（初期値は含まない）
- 空のベクタに対しては空ベクタを返す
- ワードは2つの単一要素ベクタを受け取り、1つを返す必要がある"#.to_string(),
...
[ 0 ] [ 5 ] RANGE              # → [ 0 ] [ 5 ] [ 0 1 2 3 4 5 ]
[ 1 ] [ 3 ] RANGE              # → [ 1 ] [ 3 ] [ 1 2 3 ]
[ -2 ] [ 2 ] RANGE             # → [ -2 ] [ 2 ] [ -2 -1 0 1 2 ]

## 注意
- 開始値と終了値を含む（閉区間）
- 開始値 > 終了値の場合は空ベクタ
- 元の値は消費されません"#.to_string(),
...
## 注意
- すべての型から変換可能
- 数値は分数表記（例：1/2）で文字列化されます"#.to_string(),
...
## 注意
- 数値として解釈できない文字列はエラー
- TRUEは1、FALSEは0に変換されます"#.to_string(),
...
## 使用例
[ 'NIL' ] NIL                  # → [ NIL ]
[ 'nil' ] NIL                  # → [ NIL ]（大文字小文字無視）

## 注意
- 'NIL' 以外の文字列はエラー
- NILは「値の不在」を表す特殊値です"#.to_string(),
