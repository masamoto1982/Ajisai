...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        // 暗号論的乱数生成
        ("CSPRNG", "暗号論的疑似乱数を生成｜CSPRNG → [ 0.234... ]、[ 5 ] CSPRNG → [ 0.1 0.2 ... ]（5個）", "Random"),
    ]
}
...
## 機能
暗号論的に安全な疑似乱数を [0, 1) の範囲の分数として生成します。
...
## 使用例
CSPRNG
# → [ 0.7234567890123456... ]  (64ビット精度の分数)

[ 3 ] CSPRNG
# → [ 0.123... 0.456... 0.789... ]  (3個の乱数)
...
- Stackモード (..) はサポートされません
- 生成個数は正の整数である必要があります
- 分数を個数として指定した場合は無視され、1個生成されます"#.to_string(),
