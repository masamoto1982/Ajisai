...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("SORT", "分数専用高速ソート（Introsort）｜[ 1/2 1/3 2/3 ] SORT → [ 1/3 1/2 2/3 ]", "Sorting"),
...
- 整数のみサポート"#.to_string(),
        ...
        "SORT" => r#"# SORT - 分数専用高速ソート

## 機能
ベクタまたはスタック内の数値を昇順にソートします。
Ajisaiでは全ての数値が内部的に分数（有理数）として表現されています。
SORTはこの特性を活かし、Introsortアルゴリズムと
クロス乗算による分数比較（a/b < c/d ⟺ a×d < b×c）を組み合わせることで、
高速かつ正確なソートを実現します。
