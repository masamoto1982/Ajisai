...
pub fn get_builtin_definitions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("DATETIME", "タイムスタンプを日付時刻Vectorに変換｜[ 1732531200 ] DATETIME → [ [ 2024 11 25 14 0 0 ] ]", "DateTime"),
        ("TIMESTAMP", "日付時刻Vectorをタイムスタンプに変換｜[ [ 2024 11 25 14 0 0 ] ] TIMESTAMP → [ 1732531200 ]", "DateTime"),
...
NOW DATETIME                     # → [ [ 2024 11 25 14 0 0 500/1000 ] ]
# 現在時刻を日付時刻Vectorに変換
...
- ブラウザのDate.now()を使用"#.to_string(),

        "DATETIME" => r#"# DATETIME - 日付時刻Vectorに変換

## 機能
Unixタイムスタンプをブラウザのローカルタイムゾーンでの
...
# タイムスタンプから年を取得
[ 1732531200 ] DATETIME [ 0 ] GET
# → [ 2024 ]

# 月だけを取得
[ 1732531200 ] DATETIME [ 1 ] GET
# → [ 11 ]
...
## 機能
ブラウザのローカルタイムゾーンでの日付時刻Vector
[年 月 日 時 分 秒] をUnixタイムスタンプに変換します。
実在しない日時（2023-13-32など）はエラーになります。

## 使用法
[ [ 年 月 日 時 分 秒 ] ] TIMESTAMP
→ タイムスタンプ（分数）

[ [ 年 月 日 時 分 秒 サブ秒 ] ] TIMESTAMP
→ タイムスタンプ（分数、サブ秒精度を含む）

## 使用例
[ [ 2024 11 25 14 0 0 ] ] TIMESTAMP
# → [ 1732531200 ]
# （ブラウザのタイムゾーンが Asia/Tokyo の場合）

[ [ 2024 11 25 14 0 0 1/2 ] ] TIMESTAMP
# → [ 3465062401/2 ]
# サブ秒精度（0.5秒）を含む

[ [ 1970 1 1 0 0 0 ] ] TIMESTAMP
# → [ -32400 ]
# ローカル時刻の1970-01-01 00:00:00（Asia/Tokyoの場合）

## エラーケース
[ [ 2023 13 32 0 0 0 ] ] TIMESTAMP
# エラー：実在しない日付（13月32日）

[ [ 2024 2 30 0 0 0 ] ] TIMESTAMP
# エラー：2024年2月30日は存在しない

[ [ 2024 11 25 25 0 0 ] ] TIMESTAMP
# エラー：25時は存在しない

## 活用例
# 特定の日時のタイムスタンプを取得
[ [ 2025 1 1 0 0 0 ] ] TIMESTAMP
# → 2025年1月1日0時0分0秒のタイムスタンプ

# 日付の比較（タイムスタンプ経由）
[ [ 2024 12 25 0 0 0 ] ] TIMESTAMP
[ [ 2024 12 31 0 0 0 ] ] TIMESTAMP
<
# → [ true ]（12/25 < 12/31）

# 往復変換でバリデーション
[ [ 2024 2 29 0 0 0 ] ] TIMESTAMP DATETIME
