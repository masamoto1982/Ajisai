...
pub fn op_get(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_get ==="));
    ...
    console::log_1(&JsValue::from_str(&format!("GET: index_val={:?}, target_val={:?}", index_val, target_val)));
    ...
    match target_val.val_type {
        ValueType::Vector(v) => {
            ...
            if actual_index < len {
                let result = Value { 
                    val_type: ValueType::Vector(vec![v[actual_index].clone()]) 
                };
                console::log_1(&JsValue::from_str(&format!("GET result: {:?}", result)));
...
pub fn op_insert(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_insert ==="));
    ...
    let insert_element = match element.val_type {
        ValueType::Vector(v) if v.len() == 1 => v[0].clone(),
...
pub fn op_replace(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_replace ==="));
...
pub fn op_remove(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_remove ==="));
...
pub fn op_length(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_length ==="));
        ...
        ValueType::Vector(v) => {
            ...
            let val = Value { 
                val_type: ValueType::Vector(vec![Value{ 
                    val_type: ValueType::Number(len_frac)
                }]) 
            };
...
pub fn op_take(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_take ==="));
...
pub fn op_drop_vector(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_drop_vector ==="));
    ...
    match vector_val.val_type {
        ValueType::Vector(v) => {
            ...
            let result = if count_i64 < 0 {
                let abs_count = (-count_i64) as usize;
                if abs_count > len { 
                    return Err(AjisaiError::from("Drop count exceeds vector length")); 
                }
                v[..len - abs_count].to_vec()
            } else {
...
pub fn op_repeat(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_repeat ==="));
...
pub fn op_split(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_split ==="));
    ...
    match vector_val.val_type {
        ValueType::Vector(v) => {
            ...
            for size in sizes {
                ...
                interp.workspace.push(Value { 
                    val_type: ValueType::Vector(slice) 
                });
                start = end;
            }
            Ok(())
...
pub fn op_dup_workspace(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_dup_workspace ==="));
...
pub fn op_swap_workspace(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_swap_workspace ==="));
...
pub fn op_rot_workspace(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_rot_workspace ==="));
...
pub fn op_concat(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_concat ==="));
...
pub fn op_reverse(interp: &mut Interpreter) -> Result<()> {
    console::log_1(&JsValue::from_str("=== op_reverse ==="));
