...
//!
//! OR-ELSE is the value-based, block-taking counterpart to OR-NIL. These
...
//! NIL-fallback semantics on values, and — unlike OR-NIL — its fallback is a
//! whole `{ ... }` block, so its meaning does not depend on the lexical
