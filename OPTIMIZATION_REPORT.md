...
- **実装**:
  - `simd_ops.rs` モジュールを新規作成。`try_extract_integer_vector()` で整数ベクタ判定、`try_simd_add/sub/mul()` でベクタ同士のSIMD演算、`try_simd_scalar_add/mul()` でスカラー×ベクタのSIMD演算を提供
...
| `rust/src/interpreter/higher_order.rs` | ヘルパー関数統合、スタック操作をmem::swap最適化 |
...
| `rust/src/interpreter/simd_ops.rs` | WASM SIMDアクセラレーション モジュール新規作成 |
| `rust/src/interpreter/mod.rs` | simd_ops モジュール登録 |
| `rust/src/interpreter/tensor_ops.rs` | Rc対応のVector構築に更新 |
| `rust/src/interpreter/logic.rs` | Rc対応のVector構築に更新 |
| `rust/src/interpreter/json.rs` | Rc対応のVector/JsonObject構築に更新 |
| `rust/src/wasm_api.rs` | Rc対応のイテレーション修正 |
