...
<p>Output is the only effect. <code>PRINT</code> consumes its operand and appends it to the ordered output stream; <code>KEEP</code> retains it like any other Word. Every other Word is pure: given the same stack and dictionary it produces the same result and changes nothing else.</p>
