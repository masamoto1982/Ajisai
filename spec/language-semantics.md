...
<p>A migration in progress (<code>docs/dev/semantic-spine-migration-plan.md</code>, non-canonical) is converging the host wire format on the schema below. It fixes the stack observation: a <code>protocol</code> identifier and an ordered array of observed values, bottom to top. Each observed value pairs one of the six canonical value domains (LANG.VALUES.DISJOINT) with a presentation hint, carrying <code>numerator</code>/<code>denominator</code>/<code>exact</code> for a Scalar, its text or name for a String, Boolean, or Symbol, its elements for a Vector, and its reason for a NIL. This is not yet what a caller of the shipped GUI, WASM bindings, or CLI/MCP surfaces receives: those still speak an earlier, richer wire shape carrying additional fields no source in LANG.AUTHORITY.SOURCES currently fixes. Once the migration's consumer cutover lands, this schema becomes the only channel through which anything outside the language observes the stack.</p>

<p>Every document carries a protocol version field, so a future breaking change is identifiable rather than silent. Within the current version only optional fields may be added; existing field deletion, rename, semantic change, and tuple reorder or reshape are forbidden. A breaking change raises the version, and the superseded protocol is removed rather than kept as a second reader: once current, an implementation offers exactly one protocol.</p>
