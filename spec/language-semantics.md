...
<p>There is exactly one modifier axis. By default a Word consumes the operands it reads; <code>KEEP</code> leaves them on the stack beneath the result, in their existing order. A Word whose result is empty is no exception: <code>BIND</code>, <code>DEF</code> and <code>DEL</code> perform their effect and leave their operands where they were. <code>KEEP</code> modifies a call, never a Word inside the body or block the call runs: a User Word and <code>EXEC</code> run their body consuming, and what the call reached — its operands, and for <code>EXEC</code> the block — is put back beneath what it leaves, so <code>[ 3 ] [ 1 + ] KEEP EXEC</code> is <code>[ 3 ] [ 1 + ] [ 4 ]</code>. What a call reached is known only by running it, and substituting a User Word's body for the Word is not faithful directly after <code>KEEP</code>: <code>KEEP</code> on the Word keeps the operands its call reached, while <code>KEEP [ body ] EXEC</code> keeps the block as well. An expansion of a program into Core Words alone therefore does not substitute a Word written directly after <code>KEEP</code>. The higher-order Words are the one exception: under <code>KEEP</code>, <code>MAP</code>, <code>FILTER</code>, <code>FOLD</code>, <code>SCAN</code>, <code>ANY</code>, <code>ALL</code> and <code>RANK</code> retain the collection they walk and nothing else, because the code operand states what the call is rather than being data the call read, and <code>FOLD</code>'s initial accumulator has been folded into the answer.</p>
