...
<p>Isolation is what lets a recursive <code>COND</code> word terminate cleanly — working values a clause body pushes cannot leak past it, so a tail self-call starts from one value however much scaffolding the body built — and the discard above is its cost. A block written inside another block is data where it is written, evaluated only when the Word receiving it runs it, under that Word's rule and not the enclosing block's; a self-call inside such a block is an ordinary call.</p>
...
<p>Division by zero, domain exclusion, out-of-range indexing, failed parsing, space exhaustion, and comparison-budget exhaustion (LANG.VALUES.EXACT) remain distinct reasons. The last projects to NIL tagged with the <code>TruthValue</code> role for a comparison — so it reads as UNKNOWN (LANG.VALUES.TRUTH) — and to a plain NIL for a Word whose output domain is not truth, such as an ordering-derived selection.</p>
