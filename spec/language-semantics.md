...
<p>One host protocol is current. It fixes the stack observation: a <code>protocol</code> identifier and an ordered array of observed values, bottom to top. Each observed value pairs one of the six canonical value domains (LANG.VALUES.DISJOINT) with a presentation hint, carrying <code>numerator</code>/<code>denominator</code>/<code>exact</code> for a Scalar, its text or name for a String, Boolean, or Symbol, its elements for a Vector, and its reason for a NIL. It is the only channel through which anything outside the language observes the stack.</p>
