...
<p><strong>Whole-stack.</strong> A user Word's body (<code>DEF</code>) and <code>EXEC</code> see the whole stack and leave whatever they push, however many values.</p>
<p><strong>Isolated frame.</strong> Every other block-evaluating Word runs its block alone in a frame holding a fixed number of values, and the block must leave exactly one: leaving none is ERROR, and anything below the top goes with the frame. <code>COND</code> isolates the target — one value — for every guard and the winning body. <code>MAP</code>, <code>FILTER</code>, <code>ANY</code>, and <code>ALL</code> isolate the current element — one value — and <code>FOLD</code> isolates the accumulator and the current element — two values.</p>
