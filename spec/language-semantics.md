...
<p>A block has no stack discipline of its own: the Word that evaluates it decides what the block reaches and what it may leave. The difference is whether a <code>+</code> written inside it finds two operands or none. Three rules cover every case, and one of them is written in the block's own text: a <code>DEF</code> body may begin with a <strong>parameter header</strong>, the names written before the first <code>|</code> of its first statement at its own level — <code>[ A B | A B - ] 'DIFF' DEF</code>.</p>

<div class="ref-table-wrap"><table class="ref-table"><thead><tr><th>Rule</th><th>Applies to</th><th>Frame holds</th><th>On leaving</th></tr></thead><tbody><tr><td>Declared frame</td><td>A user Word's body written with a parameter header</td><td>Nothing: the call takes exactly as many operands as the header names, deepest first, and binds them to those names</td><td>Leaves whatever it pushes, however many values; reading below the empty frame is a stack-underflow ERROR</td></tr><tr><td>Whole-stack</td><td>A user Word's body written without a header, and <code>EXEC</code></td><td>The whole stack</td><td>Leaves whatever it pushes, however many values</td></tr><tr><td>Isolated frame</td><td><code>MAP</code> <code>FILTER</code> <code>ANY</code> <code>ALL</code> — the current element · <code>FOLD</code> <code>SCAN</code> — the accumulator and the current element · <code>RANK</code> — the value reached at its depth</td><td>A fixed number of values (one, except <code>FOLD</code>'s two)</td><td>Must leave exactly one; leaving none is ERROR, and anything below the top goes with the frame</td></tr></tbody></table></div>
