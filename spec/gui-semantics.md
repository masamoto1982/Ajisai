...
<p><strong>Host commands.</strong> Some input typed into the Input surface and run is acted on by the host instead of being handed to the interpreter. A bare <code>RESET</code> discards the session, a bare <code>STACK-CLEAR</code> discards the stack, and a bare <code>EDITOR-CLEAR</code> discards the Input surface's typed text; they are the typed spelling of controls that also exist as buttons and shortcuts, and they are deliberately <em>not</em> Words — the vocabulary holds nothing that throws away the values a program was handed, the text it was typed as, or the dictionary it was defined in, and nothing in it should.</p>
