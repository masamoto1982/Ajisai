...
| word | family | summary |
|---|---|---|
| `TRUE` | booleanLogic | Push the boolean TRUE onto the stack. — e.g. `TRUE` |
| `FALSE` | booleanLogic | Push the boolean FALSE onto the stack. — e.g. `FALSE` |
| `AND` | booleanLogic | Logical AND. FALSE absorbs a NIL operand into FALSE; otherwise a NIL operand yields UNKNOWN. — e.g. `TRUE FALSE AND` |
| `NOT` | booleanLogic | Logical negation. TRUE and FALSE invert; a NIL operand (UNKNOWN) passes through unchanged. — e.g. `TRUE NOT` |
...
| `LT` | comparison | Test less-than comparison. — e.g. `1 2 LT` |
| `GT` | comparison | Test greater-than comparison. — e.g. `2 1 GT` |
| `ADD` | exactArithmetic | Add two numeric values, element-wise with broadcasting. — e.g. `1 2 ADD` |
| `SUB` | exactArithmetic | Subtract two numeric values, element-wise with broadcasting. — e.g. `5 3 SUB` |
| `MUL` | exactArithmetic | Multiply two numeric values, element-wise with broadcasting. — e.g. `2 4 MUL` |
| `DIV` | exactArithmetic | Divide two numeric values exactly (fractional result). — e.g. `10 2 DIV` |
| `FLOOR` | exactArithmetic | Round toward negative infinity. — e.g. `7/3 FLOOR` |
| `ROUND` | exactArithmetic | Round to nearest integer (half-up). — e.g. `5/2 ROUND` |
| `MIN` | exactArithmetic | Smaller of two numbers, element-wise with broadcasting. — e.g. `1 2 MIN` |
| `MAX` | exactArithmetic | Larger of two numbers, element-wise with broadcasting. — e.g. `1 2 MAX` |
| `SQRT` | exactArithmetic | Exact square root of a non-negative rational, element-wise over a vector. The result is carried in multiquadratic normal form and compares with no rounding. A negative radicand projects to NIL. — e.g. `2 SQRT` |
...
| `LENGTH` | collection | Return the number of elements in a vector. — e.g. `[ 1 2 3 ] LENGTH` |
...
| `REVERSE` | collection | Reverse the order of vector elements. — e.g. `[ 1 2 3 ] REVERSE` |
...
| `SORT` | collection | Return a copy of a vector sorted in ascending order. — e.g. `[ 3 1 2 ] SORT` |
| `ORDER` | collection | The indices that would sort a vector ascending; ties keep their original order. — e.g. `[ 30 10 20 ] ORDER` |
| `UNIQUE` | collection | The distinct elements of a vector, in first-occurrence order. — e.g. `[ 'a' 'b' 'a' ] UNIQUE` |
...
| `ZIP` | collection | Bundle equal-length vectors position by position; a matrix transposes. — e.g. `[ [ 1 2 ] [ 3 4 ] ] ZIP` |
...
| `WITHOUT` | record | A copy of a Record with one key removed: `R 'x' WITHOUT`. Removing a key the Record does not hold is not an identity but the absence `notFound` — the same discipline `GET`, `TAKE` and `PUT` keep for a position outside the Vector, so a misspelled key cannot pass silently. The other keys keep their order. A non-Record first operand is an ERROR. — e.g. `[ 'x' 'y' ] [ 1 2 ] RECORD 'x' WITHOUT` |
...
| `MAP` | higherOrder | Apply a code block to each element of a vector. — e.g. `[ 1 2 3 ] [ 2 MUL ] MAP` |
| `FILTER` | higherOrder | Keep only the elements for which a predicate block returns TRUE. — e.g. `[ 1 2 3 ] [ 2 EQ ] FILTER` |
| `FOLD` | higherOrder | Reduce a vector to a single value using an initial accumulator and combiner block. — e.g. `[ 1 2 3 ] 0 [ ADD ] FOLD` |
...
| `CHARS` | text | Split a string into a vector of one-character strings. — e.g. `'hi' CHARS` |
| `JOIN` | text | Join a vector of strings into a single string. — e.g. `[ 'h' 'i' ] JOIN` |
| `TRIM` | text | Remove whitespace from both ends of a string. — e.g. `'  hi  ' TRIM` |
...
| `NIL` | absence | Push the NIL value onto the stack. — e.g. `NIL` |
| `NIL?` | absence | Test whether the top value is an operational NIL (absent). — e.g. `1 0 DIV NIL?` |
...
| `BIND` | dictionary | Name a value for the rest of the frame that made it. One name takes the whole value; several destructure a vector of the same length, position by position. Both operands are consumed and the name pushes the value wherever it is written afterwards, however many times. A binding reaches the blocks written in its frame and never a Word called from it, and it ends when the frame does. A name already held by a Core or User Word is refused, so a name is a Word or a binding and never both. — e.g. `[ 1 2 3 ] 'XS' BIND` |
| `DEF` | dictionary | Define a user word from a body and a name. — e.g. `[ 2 MUL ] 'DOUBLE' DEF` |
| `DEL` | dictionary | Delete a User Word from the dictionary: `'INC' DEL`. A Core Word or a reserved alias is refused (`protectedWord`), a name no User Word holds is `wordNotFound`, and a Word other User Words still call is `definitionConflict` until they are deleted first. — e.g. `[ 1 ] 'W' DEF 'W' DEL` |
...
| `PRINT` | output | Write the top stack value to the output stream, consuming it. A string is written as its raw text, without the quotes the stack shows ('TEST' prints as TEST); nested strings keep their quotes. — e.g. `42 PRINT` |
